...
export default function RightSide({ playlist }) {
    const [playlists, setPlaylists] = useState();
    const [showLoading, setShowLoading] = useState(true);
    const getData = async () => {
        await API.get("/playlist/").then((response) => {
            setPlaylists(response.data.results.filter((item, index) => {
                let result = false
                item.moods.map((mood, index) => {
                    playlist.moods && playlist.moods.map((i) => {
                        result = i.id === mood.id && playlist.id !== item.id
                    })
                })
                return result;
            }));
